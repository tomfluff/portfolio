...
namespace SunnyDay.Client.Core.Models.WeatherUnderground
{
    ...
    public class WeatherRoot
    {
        ...
        [JsonIgnore]
        public string DisplayDate => (new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc).AddSeconds(double.Parse(current_observation.observation_epoch))).ToLocalTime().ToString("D");
        [JsonIgnore]
        public string DisplayTempC => $"{current_observation?.temp_c ?? 0}° {current_observation?.weather ?? string.Empty}";
        [JsonIgnore]
        public string DisplayTempF => $"{current_observation?.temp_f ?? 0}° {current_observation?.weather ?? string.Empty}";
        [JsonIgnore]
        public string DisplayIcon => current_observation?.icon_url ?? string.Empty;
        [JsonIgnore]
        public string DisplayLocation => $"{current_observation?.display_location?.full ?? "Unknown"}, {location?.country_name ?? "NAN"}";
        [JsonIgnore]
        public int UvLevel => (int)double.Parse(current_observation?.UV.Length > 0 ? current_observation.UV : "-1");
        [JsonIgnore]
        // in meterc
        public double Altitude => double.Parse(current_observation?.display_location?.elevation ?? "0");
    }
}
