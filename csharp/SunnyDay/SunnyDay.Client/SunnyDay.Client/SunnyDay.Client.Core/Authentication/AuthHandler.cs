    ...
    class AuthHandler : DelegatingHandler
    {
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            if (response.StatusCode == HttpStatusCode.Unauthorized)
            {
                ...
                var service = ServiceLocator.Instance.Resolve<ICloudService>();
            ...
            else if (response.StatusCode == HttpStatusCode.InternalServerError || response.IsSuccessStatusCode == false)
            {
                Debug.WriteLine($"{response.StatusCode}:\nis success={response.IsSuccessStatusCode}\nreason={response.ReasonPhrase}\n{await response.Content.ReadAsStringAsync()}\nrequest={request.ToString()}\ncloned request={clonedRequest.ToString()}");
