...
def runExamples():
	...
	if i % 5 == 0:
        ...
        results[i] =  getSkinColor(inP, outS, outK, outA)
    checkResults(results)

def checkResults(results):
    exp = {}
    f = open('exp_res.txt', 'r')
    f.readline()
    for line in f:
	resArr = line.split(",")
	picNum = int(resArr[0].strip())
	expSkinNum = int(resArr[1].strip())
	sunNum = int(resArr[2].strip())
	isYellow = int(resArr[3].strip())
	isBigBackground = int(resArr[4].strip())
	notWrist = int(resArr[5].strip())
	exp[picNum] = (expSkinNum, sunNum, isYellow, isBigBackground, notWrist)
    f.close()
    totalMatched = 0
    totalFail = 0
    notMatched = []
    ...
    f.write("ID,expectedSkinNum,DetectedSkinNum,isMatch,sunNum,isYellow,isBigBackground,notWrist\n")
    for i in results.keys():
	match = 0
	if exp[i][0] -1 <= results[i] <= exp[i][0] + 1:
	    match = 1
	if match == 0:
	    notMatched.append(i)
        totalMatched += match
        if match + exp[i][2] + exp[i][3] + exp[i][4] == 0:
	    totalFail += 1
	    failArr.append(i)
	f.write("{picNum},{exp},{det},{match},{sunNum},{isYellow},{isBigBackground},{notWrist}\n".format(picNum = i, exp = exp[i][0], det = results[i], match = match, sunNum = exp[i][1], isYellow = exp[i][2], isBigBackground = exp[i][3], notWrist = exp[i][4]))
    f.close()
    print "out of {numOfPics}, you got {totalMatched} matching and {totalFail} fails\nFaled on IDs:{f}\nWrongMatch on IDs:{nm}".format(numOfPics = len(results.keys()), totalMatched = totalMatched, totalFail = totalFail, f = str(failArr), nm = str(notMatched))
   ...
   # print "sim:: " + str(sum)
    for s in sum:
        if (s[1] >= maxV) and (s[0][0] > 5 and s[0][1] > 5 and s[0][2] > 5):
            ...
            maxV = s[1]
            maxC = s[0] 
    if oneBeforeMaxC is None:
        return maxC, maxC
    return maxC, oneBeforeMaxC
...
def TooBlack(skin, p):
	...
	for cell in arr:
		...
		nonBlack += 1
    #print "% of black " + str(black) + " " + str(nonBlack + black)
    if black >= (nonBlack + black)* p:
	return True
    return False
...
def getSkinColor(pathToImage, outpath_onlySkin, outpath_Kmean, outpath_AGC):
    ...
    maxSkin, oneBeforeMax = get_skin_color_from_hist(quant)
    k_color = np.zeros((200, 200, 3), np.uint8)
    k_color[:] = maxSkin
    k_color_one_before = np.zeros((200, 200, 3), np.uint8)
    k_color_one_before[:] = oneBeforeMax
    skin_type_oneBefore = categorize_skin_color(k_color_one_before[50:51,50:51])
    skin_type = categorize_skin_color(k_color[50:51,50:51])
    #if skin_type != skin_type_oneBefore:
    #    print "no match. skin_type = " + str(skin_type) + " skin_type_oneBefore = " + str(skin_type_oneBefore)
    #else:
    #    print "match. " + str(skin_type)
    ...
    if skin_type <= skin_type_oneBefore:
        ...
        cv2.putText(k_color_one_before, '#{}'.format(skin_type_oneBefore), (10, 60), cv2.FONT_HERSHEY_COMPLEX_SMALL, 3, (255,255,255),4)
        cv2.imwrite(outpath_AGC, k_color_one_before)
        return skin_type_oneBefore
